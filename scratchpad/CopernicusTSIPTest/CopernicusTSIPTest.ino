...
AltSoftSerial ss; // TX Pin 9, RX Pin 8
...
boolean dleOdd, dleLast, sendCmd;
long rate;
byte dle = 0x10;
byte etx = 0x03;
...
//  } else {
    if (sentenceRdy) {
      for ( int i = 0 ; i < sentenceBufIndex ; i++ ) {
        Serial.print( hexFill( sentenceBuf[i] ) );Serial.print(" ");
      }
      if ( sentenceBuf[1] == 0x82 ) sendCmd = true;
      Serial.println( "" );
    ...
    if ( sendCmd ) {
      sendCmd = false;
      Serial.println("0x82 rcvd");
      ss.write( dle );ss.write( 0x35 ); ss.write( dle); ss.write( etx );
    }
//  }  
}
